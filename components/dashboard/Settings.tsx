...
const Settings: React.FC<SettingsProps> = ({ fetchedDatabase, setFetchedDatabase, database }) => {
    const router = useRouter();
    const [newDatabaseName, setNewDatabaseName] = useState<string>(fetchedDatabase.title);
    ...
    return (
        <div className="space-y-4">
            <input
                ...
                className="px-3 py-2 block placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:shadow-outline-blue focus:border-blue-300"
                placeholder="Database Name"
            />
            <button
                onClick={updateDatabaseName}
                className="inline-block px-4 mx-1 py-1 text-black border border-black rounded-md"
            >
                Update
            </button>
            <button
                onClick={deleteDatabase}
                className="inline-block px-4 mx-1 py-1 text-white bg-red-500 hover:bg-red-700 rounded-md"
            >
                Delete Database
            </button>
        </div>
