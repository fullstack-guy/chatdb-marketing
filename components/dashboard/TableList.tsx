          ...
          <div key={schema} className="my-6">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3">
              {tables.map((table) => (
                <div
                  key={table.tableName}
                  className="relative flex cursor-pointer items-center rounded-lg border p-4 shadow-sm transition-transform duration-200 hover:scale-105 hover:bg-gray-100"
                  onClick={() =>
                ...
                >
                  <div
                    className="mr-2 h-8 w-8 flex-shrink-0 items-center justify-center rounded-full" // Note the "flex-shrink-0", it prevents the circle from shrinking
                    style={{
                      backgroundColor:
                  ...
                  <div className="flex flex-grow flex-col space-y-1 overflow-hidden">
                    {" "}
                    {/* Note the "flex-grow" and "overflow-hidden" */}
                    <h3
                      className="truncate text-lg font-semibold text-black"
                      style={{
