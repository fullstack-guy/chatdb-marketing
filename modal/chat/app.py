...
llm = ChatOpenAI(
    temperature=0, model="gpt-3.5-turbo-0613", openai_api_key=get_openai_api_key()
)
...
async def run_query(query_input: QueryInput):
    ...
    try:
        ...
        with get_openai_callback() as cb:
            result = agent_executor({"input": query_input.query})

            results_output = result["output"]

            # Check if results_output is a string
            if isinstance(results_output, str):
                text = results_output
            # Check if results_output is a dictionary
