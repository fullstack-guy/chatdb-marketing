...
export default function Page() {
  ...
  const connectionStringChange = (event) => {
    const result = event.target.value.replace(/^mysql:\/\//, "");
    // .replace(/^postgresql:\/\//, "");
  ...
  const getConnectionString = (fromDetails = false) => {
    ...
    if (fromDetails) {
      ...
      if (ssl) {
        baseString += '?ssl={"rejectUnauthorized":true}';
      }
    } else {
  ...
  const connectToDatabase = async (connectionStr) => {
    ...
    const url = "/fastify/api/db/mysql/connect";
    const body = {
      connection_string: "mysql://" + connectionStr,
    };
