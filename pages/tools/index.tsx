...
const ToolsPage = () => {
  const tools = [
    ...
    },
    {
      ...
      description:
        "Convert CSV files to compressed parquet. Ideal for efficient storage and analytics.",
  ...
  return (
    ...
    >
      <div className="mt-10 flex flex-col items-center p-6">
        <h1 className="mb-4 text-center text-5xl font-bold text-black md:text-left lg:text-6xl">
          Free Data Tools
        </h1>
        <p className="mb-4 text-center text-lg">
          Free tools for your data, no signup required! Everything runs in the
          browser and your data never touches our systems!
        </p>

        <div className="mt-6 grid grid-cols-1 gap-x-8 gap-y-16 md:grid-cols-2 lg:grid-cols-3">
          {tools.map((tool) => (
            <Link href={tool.link} key={tool.id}>
              <div className="transform overflow-hidden rounded-lg border border-purple-600 shadow-lg transition hover:scale-105 hover:border-purple-700">
                {" "}
                {/* Changed to an anchor tag for better semantics and added purple borders */}
